...
def load_reports() -> list[dict]:
    """Return one flat dict per run folder that contains perf_stats.json."""
    rows = []
    for folder in sorted(TRADESIMLOG.iterdir()):
        ...
        # Read model_path from the serialised params if available
        model_path = None
        ...
        rows.append({
            ...
            "trading_end":           data.get("trading_end", ""),
            "horizon_steps":         data.get("signal_horizon_steps", ""),
